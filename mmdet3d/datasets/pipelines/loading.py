...
class LoadPointsFromFile(object):
    ...
    def __call__(self, results):
        ...
        # here we use the keyframe_points to represent the point cloud loaded
        # from key frame
        results['keyframe_points'] = points.clone()
        return results
...
@PIPELINES.register_module()
class PointToMultiViewDepthForNeRF(object):
    ...
    def points2depth_and_semantic(self, points, imgs, height, width, semantic=None):
        rH, rW = imgs.shape[2], imgs.shape[3]
    ...
    def generate_single_sweep_lidar_and_semantic(self, 
                                                 lidarego2global, 
                                                 camego2global0, 
                                                 cam_info, 
                                                 cam_name, cid, 
        ...
        lidar2lidarego = np.eye(4, dtype=np.float32)
        lidar2lidarego[:3, :3] = Quaternion(
            cam_info['lidar2ego_rotation']).rotation_matrix
        lidar2lidarego[:3, 3] = cam_info['lidar2ego_translation']
        lidar2lidarego = torch.from_numpy(lidar2lidarego)
        ...
        cam2camego[:3, :3] = Quaternion(
            ...
            ['sensor2ego_rotation']).rotation_matrix
        cam2camego[:3, 3] = cam_info['cams'][cam_name][
            'sensor2ego_translation']
        cam2camego = torch.from_numpy(cam2camego)
        ...
        camego2global[:3, :3] = Quaternion(
            cam_info['cams'][cam_name]
            ['ego2global_rotation']).rotation_matrix
        camego2global[:3, 3] = cam_info['cams'][cam_name][
            'ego2global_translation']
        camego2global = torch.from_numpy(camego2global)

        cam2img = np.eye(4, dtype=np.float32)
        cam2img = torch.from_numpy(cam2img)
        cam2img[:3, :3] = torch.Tensor(cam_info['cams'][cam_name]['cam_intrinsic'])
        ...
        points_img = torch.cat(
            [points_img[:, :2] / points_img[:, 2:3], points_img[:, 2:3]],
            1)
        depth_map = self.points2depthmap(points_img.matmul(
            post_rots[cid].T) + post_trans[cid:cid + 1, :], imgs.shape[2], imgs.shape[3])
        depth_map_raw, semantic_map = self.points2depth_and_semantic(
            points_img.matmul(post_rots_noaug[cid].T) + post_trans_noaug[cid:cid + 1, :],
            imgs, self.render_size[0], self.render_size[1], lidarseg
        )
        cam2img[0, :] *= self.render_scale[1]  # 224/900
        cam2img[1, :] *= self.render_scale[0]  # 352/1600

        if previous_frame:
            cam2camego = torch.inverse(camego2global0).matmul(camego2global).matmul(cam2camego)

        return depth_map, depth_map_raw, semantic_map, cam2img, cam2camego
    ...
    def __call__(self, results):
        ...
        depth_map_list = []
        depth_nonaug_map_list = []
        semantic_map_list = []
        intricics_list = []
        pose_spatial_list = []

        num_cam = len(results['cam_names'])
        num_frame = imgs.shape[0] // num_cam
        post_rots = post_rots.view(num_frame, num_cam, *post_rots.shape[1:])
        post_trans = post_trans.view(num_frame, num_cam, *post_trans.shape[1:])
        post_rots_noaug = post_rots_noaug.view(num_frame, num_cam, *post_rots_noaug.shape[1:])
        post_trans_noaug = post_trans_noaug.view(num_frame, num_cam, *post_trans_noaug.shape[1:])

        for cid in range(len(results['cam_names'])):
            cam_name = results['cam_names'][cid]
            cam_info = results['curr']

            lidarego2global = np.eye(4, dtype=np.float32)
            lidarego2global[:3, :3] = Quaternion(
                cam_info['ego2global_rotation']).rotation_matrix
            lidarego2global[:3, 3] = cam_info['ego2global_translation']
            lidarego2global = torch.from_numpy(lidarego2global)
            ...
            depth_map, depth_map_raw, semantic_map, cam2img, cam2camego = \
                self.generate_single_sweep_lidar_and_semantic(
                    lidarego2global, camego2global, cam_info, cam_name, cid, points_lidar, imgs, lidarseg,
                    post_rots[0], post_trans[0], post_rots_noaug[0], post_trans_noaug[0], previous_frame=False,
                )
            depth_map_list.append(depth_map)
            depth_nonaug_map_list.append(depth_map_raw)
            semantic_map_list.append(semantic_map)
            intricics_list.append(cam2img)
            pose_spatial_list.append(cam2camego)

            if self.sequential:
                for seq in range(1, num_frame):
                    cam_info = results['adjacent'][seq-1]
                    depth_map, depth_map_raw, semantic_map, cam2img, cam2camego = \
                        self.generate_single_sweep_lidar_and_semantic(
                            lidarego2global, camego2global, cam_info, cam_name, cid, points_lidar, imgs, lidarseg,
                            post_rots[seq], post_trans[seq], post_rots_noaug[seq], post_trans_noaug[seq], previous_frame=True,
                        )
                    depth_map_list.append(depth_map)
                    depth_nonaug_map_list.append(depth_map_raw)
                    semantic_map_list.append(semantic_map)
        ...
        results['render_gt_depth'] = torch.stack(depth_nonaug_map_list)
        results['img_semantic'] = torch.stack(semantic_map_list)
        results['intricics'] = torch.stack(intricics_list)
        results['pose_spatial'] = torch.stack(pose_spatial_list)
        return results
...
@PIPELINES.register_module()
class PrepareImageInputsForTTA(object):
    """Prepare the image inputs for TTA pipeline.
    ...
    def __init__(
        ...
        self.is_train = False  # here we make the is_train is False
        self.data_config = data_config
        self.normalize_img = mmlabNormalize
        self.sequential = sequential
...
@PIPELINES.register_module()
class PrepareImageInputsForNeRF(object):
    """Load multi channel images from a list of separate channel files.
    ...
    def sample_noaugmentation(self, H, W, input_size=None):
        ...
        newW, newH = resize_dims
        crop_h = newH - fH
        crop_w = max(0, newW - fW)
        crop = (crop_w, crop_h, crop_w + fW, crop_h + fH)

        return resize, resize_dims, crop, False, 0.0
...
@PIPELINES.register_module()
class LoadInstanceMaskFromFile(PrepareImageInputsForNeRF):
    ...
    def __call__(self, results):
        if self.mode == 'npz':
            camera_names = [cam_name for cam_name in results['cam_names']]
            choose_cams_order_npz = ['CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT', 
                                     'CAM_BACK_LEFT', 'CAM_BACK', 'CAM_BACK_RIGHT']
            # check the choose cams order is the same with camera_names
            assert camera_names == choose_cams_order_npz[:len(camera_names)]
            
            sample_token = results['curr']['token']
            instance_mask_path = osp.join(self.instance_mask_dir, 
                                          f"{sample_token}.npz")
            
            # (6, H, W), from uint8 to int64
            ...
            origin_h, origin_w = instance_mask_all_cams.shape[1:]
            if origin_h != self.render_size[0] or origin_w != self.render_size[1]:
                instance_mask_all_cams = self.transform(instance_mask_all_cams)

            # -1 means we donot detect any instances in these regions
            instance_imgs = instance_mask_all_cams - 1
        elif self.mode == 'json':
            ## NOTE: here we load the .json file which generated by SAM model to
            # represent the instance segmentation results.
            instance_imgs = []
